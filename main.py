...
now = datetime.now()
days = now.strftime('%d/%b/%Y')

x = 1
while x:
    text = int(input("Enter 1 for entry and 2 for exit"))
    if text == 1:
        webcam = cv2.VideoCapture(0, cv2.CAP_DSHOW)  # to capture form webcam
        while True:
            (_,
            ...
            for (x, y, w, h) in faces:
                ...
                if prediction[1] < 500:
                    ...
                    ex.make_attendance_entry(days, name)
                else:
            ...
            for (x, y, w, h) in faces:
                ...
                if prediction[1] < 500:
                    ...
                    cv2.putText(im, '% s ' % (names[prediction[0]]), (x - 10, y - 10),
                                cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0))
                    name = names[prediction[0]]
                    exi.make_attendance_exit(days, name)
                else:
                    cv2.putText(im, 'not recognized', (x - 10, y - 10), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0))
    ...
    x = int(input("press 1 to continue and 0 to exit"))
